...
import torch.optim as optim  # not used yet, but we will
...
BASE_MODEL_NAME = "conv_td0_bjarni"
...
#2. Encoder constants and single-board encoder
# === Encoding constants ===
...
class ConvValueNet(nn.Module):
    def __init__(
        ...
        # After convs we pool to a single vector of size conv_channels,
    ...
    def forward(self, pts: torch.Tensor, gfeats: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.tanh(self.fc2(x))  # squash to (-1,1) as game outcome
        return x.squeeze(-1)
#End of 4.
...
def play_one_selfplay_game(
    ...
    Args:
        net: ConvValueNet used by both players (shared parameters).
...
def play_game_and_fill_replay(
    ...
    while True:
        ...
        if move is None:
            if verbose:
                print("  No legal moves, passing.")
            # For learning we *can* still store this as a transition with reward 0
            replay.push(
                Transition(
                    board=board.copy(),
                    player=player,
...
def make_networks() -> tuple[ConvValueNet, ConvValueNet]:
    online = ConvValueNet().to(DEVICE)
    target = ConvValueNet().to(DEVICE)
...
def train_step_td0(
    online_net: ConvValueNet,
    target_net: ConvValueNet,
    optimizer: optim.Optimizer,
    replay: ReplayBuffer,
    batch_size: int,
    gamma: float = 0.99,
...
def train_td0_conv_main(
    ...
    # --- dump eval history to CSV for later plotting ---
...
if __name__ == "__main__":
    # 1) Quick smoke test to ensure nothing is broken
    _smoke_test()

    # 2) Optional very short TD(0) debug run
    #    You can comment this out once you trust the pipeline.
    # print("\n[TD0-DEBUG] Starting small online TD(0) training run...")
    # debug_train_online_td0()

    # 3) Final long run (250k by default)
